A=rgb2gray(imread("Image.jpeg"));
[R C] = size(A); %taking size of image A
B=imnoise(A,"salt & pepper",0.1);%if you want to add salt noise
medianfilteredImage = zeros(R,C); %creating a matrix of size A 
...
for i=ceil(filterSize(1)/2):1:R-(floor(filterSize(1)/2))
    for j = ceil(filterSize(1)/2):1:C-(floor(filterSize(2)/2))
        ...
        % from (3,3) of image B
         ...
         medianfilteredImage(i,j) =median(myNR); % placing this sum at filterimage(i,j)
    end
end
figure; imshow(B,[])
