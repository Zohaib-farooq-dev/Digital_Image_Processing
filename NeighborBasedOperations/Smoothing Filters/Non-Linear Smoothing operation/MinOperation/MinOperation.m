...
B=imnoise(A,"salt",0.1);%if you want to add salt noise
minfilteredImage = zeros(R,C); %creating a matrix of size A 
...
for i=ceil(filterSize(1)/2):1:R-(floor(filterSize(1)/2))
    for j = ceil(filterSize(1)/2):1:C-(floor(filterSize(2)/2))
        ...
        % from (3,3) of image B
         ...
         sortedmyNR=sort(myNR);
         minfilteredImage(i,j) =sortedmyNR(1); % placing this sum at filterimage(i,j)
    end
end
%figure=imshow(B,[])
figure=imshow(minfilteredImage,[]);     