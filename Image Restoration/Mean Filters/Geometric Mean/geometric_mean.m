...
for i=ceil(filterSize(1)/2):1:R-(floor(filterSize(1)/2))
    for j = ceil(filterSize(1)/2):1:C-(floor(filterSize(2)/2))
        ...
        % from (3,3) of image A
         ...
         Temp = double(myTemp);
         tempR = reshape(Temp,1,[]); %converting 2d array into 1d 
         product = prod(tempR); %multiplying all elements of the array 
         geometric = nthroot(product,filterSize(1) * filterSize(1)); % taking nth root like 9th (1/9) root if filter size = 3
         
         denoisedImage(i,j) = geometric; % placing this sum at filterimage(i,j)
    end
end
