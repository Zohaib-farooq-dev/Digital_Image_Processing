...
for i=ceil(filterSize(1)/2):1:R-(floor(filterSize(1)/2))
    for j = ceil(filterSize(1)/2):1:C-(floor(filterSize(2)/2))
        ...
        % from (3,3) of image A
         ...
         Temp = double(myTemp);
         reciprocals = 1 ./ Temp; % calculating reciprocal of each element 
         sum_reciprocals = sum(reciprocals(:)) ;% adding all the reciprocals 
         harmonic = ( filterSize(1) * filterSize(2)) / sum_reciprocals; % dividing the m * n with the sum of reciprocals 
         
         denoisedImage(i,j) = harmonic; % placing this sum at filterimage(i,j)
    end
end
